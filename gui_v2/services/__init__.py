"""GUI-v2-owned service package with lazy public exports.
...
def __getattr__(name: str) -> Any:
    ...
    if name in _LOCATION_EXPORTS:
        from gui_v2.services import location_service as module

        return getattr(module, name)
    if name in _MAP_EXPORTS:
        from gui_v2.services import map_asset_server as module

        return getattr(module, name)
    if name in _HANDOFF_EXPORTS:
        from gui_v2.services import planner_handoff_service as module

        return getattr(module, name)
    if name in _EXPORT_SERVICE_EXPORTS:
        from gui_v2.services import prediction_export_service as module

        return getattr(module, name)
    if name in _PREFERENCES_EXPORTS:
        from gui_v2.services import preferences_service as module

        return getattr(module, name)
    if name in _RUNTIME_PATH_EXPORTS:
        from gui_v2.services import runtime_paths as module
