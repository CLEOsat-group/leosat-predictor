"""Styled hierarchical navigation sidebar for GUI v2."""
...
from gui_v2.shell.navigation_model import (
    NavigationDomain,
    NavigationNode,
    breadcrumb_for_page,
...
from gui_v2.styles.icon_provider import (
    register_icon_host,
    set_themed_icon,
    themed_icon,
)
...
class NavigationSidebar(QWidget):
    ...
    def __init__(self, nodes: tuple[NavigationNode, ...], parent: QWidget | None = None) -> None:
        ...
        self.setObjectName("guiV2NavigationRail")
        self._nodes = nodes
        ...
        self._domain_buttons: dict[str, QToolButton] = {}
        self._domain_menu_actions: dict[str, QAction] = {}
        # Remembers the last concrete page viewed in each domain (by domain
        # label) so re-entering a domain restores it instead of the default
        # sub-page.  Session-scoped; not persisted across restarts.
        self._last_page_by_domain: dict[str, str] = {}

        self._title = QLabel("NAVIGATION", self)
        self._title.setObjectName("guiV2NavigationTitle")

        self._primary_tree = self._create_tree("guiV2NavigationTree")
        self._bottom_tree = self._create_tree("guiV2NavigationBottomTree")
        # Retain the historical attribute as an alias for the primary tree.
        self._tree = self._primary_tree

        self._expanded = QWidget(self)
        self._expanded.setObjectName("guiV2ExpandedNavigation")
        ...
        self._separator.setObjectName("guiV2NavigationSeparator")
        ...
        self._rail.setObjectName("guiV2DomainRail")
        ...
        self._stack.setObjectName("guiV2NavigationModeStack")
    ...
    def set_current_page_context(self, page_id: str) -> None:
        ...
        self._remember_domain_page(page_id)
    ...
    def _create_domain_button(self, domain: NavigationDomain) -> QToolButton:
        """Create one compact-rail domain control.

        The button body keeps the one-click "open the domain's default page"
        behavior.  When a domain has more than one page, the button also gets a
        dropdown menu (``MenuButtonPopup``) listing every sub-page so they remain
        reachable while the sidebar is collapsed.
        """

        button = QToolButton(self._rail)
        button.setObjectName("guiV2RailButton")
        button.setText(domain.label)
        ...
        if len(domain.page_ids) > 1:
            menu = QMenu(button)
            menu.setObjectName("guiV2RailMenu")
            for page_id in domain.page_ids:
                label = breadcrumb_for_page(self._nodes, page_id)[-1]
                action = QAction(label, menu)
                action.setCheckable(True)
                action.triggered.connect(
                    lambda _checked=False, target=page_id: self.select_page(target)
                )
                menu.addAction(action)
                self._domain_menu_actions[page_id] = action
            button.setMenu(menu)
    ...
    def _handle_current_item_changed(
        ...
        leaf_page_id = current.data(0, _PAGE_ID_ROLE)
        if leaf_page_id:
            page_id = leaf_page_id
        else:
            # Branch row: enter the domain's last-viewed page instead of always
            # the first descendant.
            default_page_id = current.data(0, _DEFAULT_PAGE_ID_ROLE)
            page_id = self._domain_entry_page(str(default_page_id)) if default_page_id else None
        if page_id:
            page_id_text = str(page_id)
            self._current_page_id = page_id_text
            self._remember_domain_page(page_id_text)
            self._update_domain_buttons()
            self.pageSelected.emit(page_id_text)

    def _handle_domain_clicked(self, domain: NavigationDomain) -> None:
        target_page_id = self._domain_entry_page(domain.default_page_id)
        self.set_current_page_context(target_page_id)
        self.pageSelected.emit(target_page_id)

    def _remember_domain_page(self, page_id: str) -> None:
        """Record ``page_id`` as the last-viewed page for its domain."""

        domain = domain_for_page(self._domains, page_id)
        if domain is not None:
            self._last_page_by_domain[domain.label] = page_id

    def _domain_entry_page(self, default_page_id: str) -> str:
        """Return the page to open when entering the domain owning a default page.

        Prefers the domain's last-viewed page (when still valid) so switching
        away from and back to a workflow restores the sub-page the user left,
        falling back to ``default_page_id`` for never-visited domains.
        """

        domain = domain_for_page(self._domains, default_page_id)
        if domain is not None:
            remembered = self._last_page_by_domain.get(domain.label)
            if remembered and domain.contains(remembered):
                return remembered
    ...
    def _update_domain_buttons(self) -> None:
        ...
        for domain in self._domains:
            ...
            repolish(button)
        for page_id, action in self._domain_menu_actions.items():
            action.setChecked(page_id == self._current_page_id)
