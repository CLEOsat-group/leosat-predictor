...
def _terminate_process_group(pid, sig=signal.SIGTERM, wait=5.0):
    if IS_WINDOWS:
        # Windows: best-effort terminate by PID (no os.setsid/os.killpg semantics).
        try:
    ...
    # POSIX path (Linux/macOS): keep original process-group behavior.
    try:
        pgid = os.getpgid(pid)
        os.killpg(pgid, sig)
    except ProcessLookupError:
...
def cleanup(signum, frame):
    ...
    for proc in processes:
        try:
            print(f"Terminating process for: {proc.args}")
            _terminate_process_group(proc.pid, sig=signal.SIGTERM, wait=2.0)
        except Exception as e:
            print(f"Error terminating process {proc.args}: {e}")

    # Linux/macOS-only force kill of leftover gunicorn workers and zombie reaping.
    if not IS_WINDOWS:
        try:
...
if __name__ == "__main__":
    ...
    if is_port_in_use(8000):
        ...
        if IS_WINDOWS:
            print("Starting Flask server on Windows...")
            server_args = [
                sys.executable,
                "-m", "flask",
                "--app", "src.main:app",
