"""GUI Observation Planner coordinator.
...
plan records while the GUI pages stay thin view adapters.
"""
...
from gui.helpers.planner_export_naming import (
...
from gui.state.planner_state import (
...
class PlannerController(QObject):
    """Own GUI planner runtime state, workers, and row synchronization."""
    ...
    def set_preferences(self, preferences: Mapping[str, Any] | ObservationPlannerPreferences | None) -> None:
        """Set planner runtime preferences from the GUI preference tree."""
    ...
    def _mark_diagnostics_stale_after_plan_edit(self) -> None:
        ...
        responsive. GUI follows the same rule: plan rows and checkboxes update
    ...
    def export_current_plan(
        ...
        *,
        ...
        coordinate_format : str, optional
            RA/DEC output-format identifier.  When omitted, the current GUI
    ...
    def _on_plan_worker_progress(self, message: str) -> None:
        """Forward worker progress to GUI logging and progress signals."""
        if not message:
            return
