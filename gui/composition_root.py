"""Composition root for the production GUI application.
...
from gui.services.runtime_paths import GuiRuntimePaths
from gui.shell.navigation_model import build_navigation_tree
...
@dataclass(frozen=True)
class Composition:
    """Top-level composed objects for GUI."""
...
def build_services(runtime_paths: GuiRuntimePaths | None = None) -> ServiceBundle:
    ...
    _logger.debug(
        "Building GUI service bundle (frozen=%s, resources=%s, user_data=%s).",
        paths.frozen,
        paths.resource_root,
...
def create_tle_service_from_preferences(
    ...
    *,
    ...
    """Create a GUI TLE service from saved preferences.
...
def create_composition(runtime_paths: GuiRuntimePaths | None = None) -> Composition:
    """Create the GUI main window and non-visual service bundle.
    ...
    _logger.info("GUI composition created with %d navigation sections.", len(sections))
