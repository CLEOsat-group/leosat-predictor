# GUI Replacement Path

`gui` is the clean replacement-path GUI package for the LEO satellite predictor.
...
GUI is the active replacement-path baseline. It provides the dashboard shell,
...
The GUI baseline launches with:
...
gui/
    ...
    widgets/              reusable live GUI widgets
...
GUI now includes a footer-level status/event strip.  The strip is a global,
...
```text
gui/state/status_event.py
gui/widgets/status_event_strip.py
```
...
indicate that GUI currently runs background workflows.


## Task 43I async-task contract

GUI now includes a Qt-free contract for future long-running task reporting.
