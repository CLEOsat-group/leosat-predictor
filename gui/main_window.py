"""Main window for the GUI baseline."""
...
from gui.services.planner_handoff_service import PlannerHandoffError

from gui.controllers import PlannerController
from gui.shell import DashboardShell, NavigationNode
from gui.shell.navigation_model import iter_pages
from gui.state import StatusEvent
from gui.state.overpass_results_state import OverpassResultsState
from gui.state.preferences_state import PreferenceSavePlan
from gui.state.precise_results_state import PreciseResultsState
...
class _ServiceBundle(Protocol):
    """Structural service-bundle protocol consumed by the GUI window."""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *, sections: tuple[NavigationNode, ...], services: _ServiceBundle) -> None:
        ...
        self.setObjectName("guiRoot")
        self.setWindowTitle("LEO Satellite Predictor — GUI")
        ...
        for page in iter_pages(sections):
            ...
            elif page.page_id == "configuration.observatory":
                widget = ConfigurationObservatoryPage(
                    page=page,
                    preferences_service=self._services.preferences_service,
                    parent=self._shell,
                )
            elif page.page_id == "configuration.tle":
                widget = ConfigurationTlePage(
                    page=page,
                    preferences_service=self._services.preferences_service,
                    parent=self._shell,
                )
            ...
            elif page.page_id == "configuration.interface":
                widget = ConfigurationInterfacePage(
                    page=page,
                    preferences_service=self._services.preferences_service,
                    parent=self._shell,
                )
            elif page.page_id == "configuration.paths":
                widget = ConfigurationPathsPage(
                    page=page,
                    preferences_service=self._services.preferences_service,
                    parent=self._shell,
                )
    ...
    def _handle_preferences_saved(self, plan: PreferenceSavePlan) -> None:
        """Apply saved preferences to eligible GUI runtime components."""
        messages: list[str] = []
        ...
        message = "Preferences saved and applied to eligible GUI pages."
        if not messages:
    ...
    def _apply_theme(self, theme: str) -> bool:
        """Apply the configured GUI stylesheet to the running application."""
        ...
        try:
            ...
            _logger.warning("Unable to apply GUI theme '%s'.", theme, exc_info=True)
            return False
        return True
    ...
    def _refresh_tle_service(self, preferences: Mapping[str, Any]) -> object | None:
        ...
        if not callable(factory):
            _logger.warning("TLE preferences saved but no GUI TLE service factory is available.")
            return None
        try:
            self._current_tle_service = factory(preferences)
        except Exception:  # pragma: no cover - file/service construction guard
            _logger.warning("Unable to refresh GUI TLE service after preferences save.", exc_info=True)
            return None
    ...
    def _refresh_configuration_status_pages(self) -> None:
        ...
        for page in self._configuration_pages:
            if page.objectName() == "guiConfigurationPage" and page is not self.sender():
    ...
    def _configure_planner_preferences_from_preferences(self) -> None:
        """Initialize GUI planner runtime defaults from saved preferences."""
        ...
        try:
            ...
            _logger.debug("Unable to load preferences for GUI planner defaults.", exc_info=True)
            return
        self._planner_controller.set_preferences(preferences)


    def _handoff_precise_results_to_planner(self) -> bool:
        """Send completed precise results directly to the GUI planner."""
        ...
        _logger.info(
            "Sent precise prediction results to GUI Observation Planner: %d row(s), %d column(s).",
            payload.row_count,
    ...
    def _configure_observatory_context_from_preferences(self) -> None:
        ...
        try:
            ...
            _logger.debug("Unable to load preferences for GUI time context.", exc_info=True)
            return
    ...
    def closeEvent(self, event: QCloseEvent) -> None:  # type: ignore[override]
        """Stop live GUI header timers before the window closes."""
        _logger.debug("GUI window closing, releasing runtime resources.")
