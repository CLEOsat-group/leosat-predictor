"""GUI adapter for the modular observation-planner visibility table."""
...
from gui.widgets.observation_visibility_table import ObservationVisibilityTable


class PlannerVisibilityTable(ObservationVisibilityTable):
    """GUI owned adapter around the existing model/view visibility table.
    ...
    large-table column-width behavior while preserving a clear GUI import
    boundary for later promotion work.
    """

    def __init__(self, parent=None):
        """Initialize the GUI table adapter."""
    ...
    def focus_row(self, row: int) -> None:
        """Scroll to and select one model row using GUI/PyQt6 semantics.
        ...
        Qt5-era selection-flag spelling. GUI must not patch that file during
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:  # noqa: N802
        """Toggle the checkbox column explicitly for GUI users.

        The inherited model already exposes a checkable ``_checked`` column, but
        relying entirely on the default delegate made the GUI checkbox path
        fragile under row-selection styling. Handling clicks in the adapter keeps
        the fix inside GUI and still routes the change through ``setData`` so
